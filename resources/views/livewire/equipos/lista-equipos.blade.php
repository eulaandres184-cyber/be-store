...
<div class="productos-filtros">
    <input class="bs-input" wire:model.live.debounce.300ms="busqueda"
           placeholder="Buscar por marca, modelo, IMEI o color..."/>
...
<div class="bs-card" style="padding:0;overflow:hidden">
    <table class="bs-table">
        <thead>
            <tr>
                <th class="th-sort" wire:click="ordenar('marca')">
                    Marca/Modelo @if($ordenarPor==='marca'){{ $direccion==='asc'?'↑':'↓' }}@else<span class="th-icon">↕</span>@endif
                </th>
                <th class="th-sort" wire:click="ordenar('imei')">
                    IMEI @if($ordenarPor==='imei'){{ $direccion==='asc'?'↑':'↓' }}@else<span class="th-icon">↕</span>@endif
                </th>
                <th>Detalle</th>
                <th class="th-sort" wire:click="ordenar('precio_usd')">
                    Precio USD @if($ordenarPor==='precio_usd'){{ $direccion==='asc'?'↑':'↓' }}@else<span class="th-icon">↕</span>@endif
                </th>
                <th>Precio ARS</th>
                <th class="th-sort" wire:click="ordenar('estado')">
                    Estado @if($ordenarPor==='estado'){{ $direccion==='asc'?'↑':'↓' }}@else<span class="th-icon">↕</span>@endif
                </th>
                <th></th>
            </tr>
        </thead>
        ...
        <tr wire:key="eq-{{ $equipo->id }}">
            <td>
                ...
                @if($equipo->capacidad_gb)
                    <div style="font-size:.72rem;color:var(--bs-muted)">{{ $equipo->capacidad_gb }}GB</div>
                @endif
            </td>
            <td style="font-family:var(--bs-font-mono);font-size:.78rem">
                {{ $equipo->imei ?? '—' }}
            </td>
            <td>
                <div style="font-size:.78rem">
                    @if($equipo->color)
                        <span class="bs-badge-blue">{{ $equipo->color }}</span>
                    @endif
                    <span class="bs-badge-{{ $equipo->condicion==='nuevo'?'green':($equipo->condicion==='usado'?'amber':'blue') }}"
                          style="margin-left:2px">
                        {{ ucfirst($equipo->condicion) }}
                    </span>
                    @if($equipo->bateria_pct)
                        <div style="margin-top:3px">🔋 {{ $equipo->bateria_pct }}%</div>
                    @endif
                </div>
            </td>
            <td style="font-weight:700;color:var(--bs-blue)">
                u$s {{ number_format($equipo->precio_usd, 0, ',', '.') }}
            </td>
            <td style="font-weight:600">
                ${{ number_format($equipo->precio_usd * $this->dolar, 0, ',', '.') }}
            </td>
            <td>
